# Rule: Explore AI Coding Agent Source Code
...
Explore source code for ANY question about these tools, including:
...
Do NOT answer from memory. Even if you think you know the answer, verify it by exploring the code. Your assumptions are likely wrong or outdated.
...
- Transport layer implementation (stdio, SSE)
- Server lifecycle management

When the first search yields incomplete results, launch follow-up searches. Explore adjacent files. Read full implementations, not just snippets. The goal is comprehensive understanding, not a quick answer.

## Proof of Verification

After researching, every statement in your response must be tied to a specific code location as evidence that it was actually checked. Use concrete citations that point to file paths and line numbers (or a short quoted snippet) for each factual claim. If you cannot find a code location to support a statement, say so explicitly instead of guessing.
...
In a╳kit docs, unqualified “sandbox” means the external sandbox (`axsandbox` container).

- **External sandbox / `axsandbox` container**: OS-level isolation via an a╳kit-managed Podman container.
- **Approval mode (permission prompting mode)**: whether an agent prompts vs auto-approves/auto-denies; not isolation.
- **Agent internal sandbox**: sandbox/isolation built into an agent CLI (separate from the external sandbox).

Agent notes:

- **Codex internal sandbox**: controlled by Codex (e.g., `--sandbox ...`); a╳kit `axrun --sandbox` means “run inside an `axsandbox` container”.
- **Gemini internal sandbox**: controlled by Gemini (e.g., `GEMINI_SANDBOX=...`); `--yolo` is approval mode, not isolation.
- **Claude Code**: “dontAsk”/permissions modes are approval mode, not isolation.
...
| **axsandbox** | External sandbox engine (Podman container sessions) |
...
1. **Push**: `axauth vault push --agent <agent> --name <name>`
   - For OpenCode: `axauth vault push --agent opencode --provider <provider> --name <name>`
...
1. **Export**: `axauth export --agent <agent> --output creds.json --no-password`
   - For OpenCode: `axauth export --agent opencode --provider <provider> --output creds.json --no-password`
